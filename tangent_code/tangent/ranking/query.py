...
class Query:
    ...
    def __init__(self, name, expression, mathml=None, initRetrievalTime='undefined', max_results=0):
        self.name = name
        ...
        for group in self.sorted_results:
            for subgroup in group:
                ...
                for expression in subgroup:
                    result = self.results[expression]
                    for doc_id, location in result.locations:
                        csv_writer.writerow(["R", str(doc_id), str(location), result.expression, str(result.new_scores[0])])
