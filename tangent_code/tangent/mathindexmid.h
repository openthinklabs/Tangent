...
class ExprNode { public:
  ...
  bool heightGT(int h) { if (h<=0&&chSize>0) { return true; }  for (int i=0; i<chSize; i++) { if (ch[i]->heightGT(h-1)) { return true; } } return false; }
  ...
  void tuples(TupleCB& cb, int w, bool eob, string loc, ExprNode* from, string rel) { rel+=ty; if (ty!='w' || te!="E!") { cb.tuple(from->te,te,rel,runl(loc)); } w--; if (w<=0) return; for (int i=0; i<chSize; i++) { ch[i]->tuples(cb,w,eob,loc,from,rel); } }
  void tuples(TupleCB& cb, int w, bool eob, string loc) { loc+=ty; for (int i=0; i<chSize; i++) { ch[i]->tuples(cb,w,eob,loc,this,""); } if (!children('n') && te!="E!") { if (eob) { cb.tuple(te,"0!","n",runl(loc)); } } for (int i=0; i<chSize; i++) { ch[i]->tuples(cb,w,eob,(ty=='-'?"":loc)); } }
